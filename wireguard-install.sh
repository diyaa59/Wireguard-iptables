...
PostDown = firewall-cmd --remove-port ${SERVER_PORT}/udp && firewall-cmd --remove-rich-rule='rule family=ipv4 source address=${FIREWALLD_IPV4_ADDRESS}/24 masquerade' && firewall-cmd --remove-rich-rule='rule family=ipv6 source address=${FIREWALLD_IPV6_ADDRESS}/24 masquerade'" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
		...
		echo "PostUp = iptables -A FORWARD -i ${SERVER_PUB_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; iptables -A FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; iptables -t nat -A POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE; ip6tables -A FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; ip6tables -t nat -A POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE; iptables -A FORWARD -i ${SERVER_WG_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; ip6tables -A FORWARD -i ${SERVER_WG_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; iptables -N WIREGUARD; iptables -I INPUT 1 -j WIREGUARD; ip6tables -N WIREGUARD; ip6tables -I INPUT 1 -j WIREGUARD; iptables -I WIREGUARD 1 -i ${SERVER_WG_NIC} -j ACCEPT; ip6tables -I WIREGUARD 1 -i ${SERVER_WG_NIC} -j ACCEPT; iptables -I WIREGUARD 1 -i ${SERVER_PUB_NIC} -p udp --dport ${SERVER_PORT} -j ACCEPT; ip6tables -I WIREGUARD 1 -i ${SERVER_PUB_NIC} -p udp --dport ${SERVER_PORT} -j ACCEPT
PostDown = iptables -D FORWARD -i ${SERVER_PUB_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; iptables -D FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; iptables -t nat -D POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE; ip6tables -D FORWARD -i ${SERVER_WG_NIC} -j ACCEPT; ip6tables -t nat -D POSTROUTING -o ${SERVER_PUB_NIC} -j MASQUERADE; iptables -D FORWARD -i ${SERVER_WG_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; ip6tables -D FORWARD -i ${SERVER_WG_NIC} -o ${SERVER_WG_NIC} -j ACCEPT; iptables -D WIREGUARD -i ${SERVER_WG_NIC} -j ACCEPT; ip6tables -D WIREGUARD -i ${SERVER_WG_NIC} -j ACCEPT; iptables -D WIREGUARD -i ${SERVER_PUB_NIC} -p udp --dport ${SERVER_PORT} -j ACCEPT; ip6tables -D WIREGUARD -i ${SERVER_PUB_NIC} -p udp --dport ${SERVER_PORT} -j ACCEPT; iptables -D INPUT -j WIREGUARD; iptables -X WIREGUARD; ip6tables -D INPUT -j WIREGUARD; ip6tables -X WIREGUARD" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	fi
